...
import { ErrorKind, ErrorValue, ExpressionError, FormulaValue, NamedValue } from '../../public'
...
export class ResolvedObjectHelpers {
  ...
  public static ResolvedObjectError(node: ResolvedObjectNode): FormulaValue {
    return new ErrorValue(
      node.IRContext,
      new ExpressionError(`Unrecognized symbol ${node?.Value}`, node.IRContext.sourceContext, ErrorKind.Validation),
    )
  }
}
