...
import { RuntimeHelpers } from './functions/RuntimeHelpers'
...
export class RecalcEngineWorker implements IScope {
  // recomputed values.
  ...
  // Just recalc Name.
  private async recalcWorker2(name: string) {
    ...
    if (fi.binding != null) {
      ...
      const newValue = await irnode.accept(v, SymbolContext.New())
