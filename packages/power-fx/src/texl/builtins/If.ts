...
import { CallNode, FirstNameNode, ListNode, TexlNode, VariadicOpNode } from '../../syntax';
...
export class IfFunction extends BuiltinFunction {
  ...
  public getSignatures() {
    ...
    return [
      ...
      [TexlStrings.IfArgCond, TexlStrings.IfArgTrueValue, TexlStrings.IfArgElseValue],
      [
  ...
  public checkInvocation(
    ...
    for (let i = 0; i < (count & ~1); i += 2) {
      let CheckType = super.checkType(args[i], argTypes[i], DType.Boolean, errors, true);
  ...
  private TryGetDSNodes(binding: TexlBinding, args: TexlNode[]): [boolean, Array<FirstNameNode>] {
    ...
    for (let i = 1; i < count; ) {
      ...
      let TryGetValidValue = ArgValidators.DataSourceArgNodeValidator.tryGetValidValue(
        nodeArg,
        binding
      );
