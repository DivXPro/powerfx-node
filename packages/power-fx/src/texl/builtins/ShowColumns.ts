...
export class ShowColumnsFunction extends FunctionWithTableInput {
  ...
  public checkInvocation(
    ...
    // let isValidInvocation = CheckInvocation(args, argTypes, errors, out returnType, out nodeToCoercedTypeMap);
    ...
    for (let i = 1; i < count; i++) {
      ...
      if (!GetType2[0]) {
        // if (colsToKeep.TryGetType(columnName, out let existingColumnType))
        isValidInvocation = false
        errors.ensureErrorWithSeverity(
