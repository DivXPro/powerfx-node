...
export interface IIntellisenseData {
  ...
  /// </returns>
  // out int newHighlightStart, out int newHighlightEnd, out string newParamName, out string newInvariantParamName
  TryAugmentSignature(
    func: TexlFunction,
    argIndex: number,
    paramName: string,
  ...
  /// </returns>
  GenerateParameterDescriptionSuffix(func: TexlFunction, paramName: string): string
}
