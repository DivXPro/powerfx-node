...
import { DottedNameNode, FirstNameNode, StrLitNode, TexlNode } from '../../syntax'
...
export class SortOrderValidator implements IArgValidator<string> {
  public tryGetValidValue(argNode: TexlNode, binding: TexlBinding): [boolean, string] {
    return this.tryGetValidSortOrder(argNode, binding)
  }

  private tryGetValidSortOrder(argNode: TexlNode, binding: TexlBinding): [boolean, string] {
    ...
    switch (argNode.kind) {
      case NodeKind.FirstName:
        return this.tryGetValidSortOrderNodeForFirstName(argNode.asFirstName(), binding)
      case NodeKind.DottedName:
        return this.tryGetValidSortOrderNodeForDottedName(argNode.asDottedName(), binding)
  ...
  private tryGetValidSortOrderNodeForDottedName(node: DottedNameNode, binding: TexlBinding): [boolean, string] {
  ...
  private tryGetValidSortOrderNodeForFirstName(node: FirstNameNode, binding: TexlBinding): [boolean, string] {
    // Contracts.AssertValue(node);
    // Contracts.AssertValue(binding);
  ...
  private TryGetValidSortOrderNodeForStrLit(node: StrLitNode): [boolean, string] {
    // Contracts.AssertValue(node);
  ...
  private verifyFirstNameNodeIsValidSortOrderEnum(node: FirstNameNode, binding: TexlBinding): boolean {
    // Contracts.AssertValue(node);
    // Contracts.AssertValue(binding);
    ...
    const result = binding.nameResolver.tryLookupEnum(new DName(LanguageConstants.SortOrderEnumStringInvariant))
