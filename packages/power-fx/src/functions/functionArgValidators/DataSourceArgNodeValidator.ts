...
export class DataSourceArgNodeValidator implements IArgValidator<Array<FirstNameNode>> {
  public tryGetValidValue(argNode: TexlNode, binding: TexlBinding): [boolean, Array<FirstNameNode>] {
    ...
    switch (argNode.kind) {
      case NodeKind.FirstName:
        const result = this.tryGetDsNodeForFirstName(argNode.asFirstName(), binding)
  ...
  private tryGetDsNodes(callNode: CallNode, binding: TexlBinding): [boolean, Array<FirstNameNode>] {
  ...
  private tryGetDsNodeForFirstName(firstName: FirstNameNode, binding: TexlBinding): [boolean, FirstNameNode] {
    ...
    if (binding.entityScope == null || !binding.entityScope.tryGetEntity(firstNameInfo.name)[0]) {
      return [false, dsNode]
    }
