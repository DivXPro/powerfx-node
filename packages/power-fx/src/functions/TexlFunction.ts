...
import { BinaryOpNode, CallNode, FirstNameNode, Identifier, TexlNode } from '../syntax';
...
import { FunctionInfo, FunctionSignature, ParameterInfo } from './transportSchemas';
...
export abstract class TexlFunction implements IFunction {
  ...
  public get helpLink() {
    ...
    return (
      StringResources.Get('FunctionReference_Link') + this.localeInvariantName[0].toLowerCase()
    );
  }
  ...
  constructor(
    ...
    if (!isNullOrEmpty(localeSpecificName)) {
      this.localeSpecificNamespace = new DPath().append(new DName(localeSpecificName));
  ...
  // TODO:
  protected getUniqueTexlRuntimeNameInner(suffix: string, suppressAsync = false) {
  ...
  public checkForDynamicReturnType(binding: TexlBinding, args: TexlNode[]): boolean {
    return false;
  }
  ...
  public supportCoercionForArg(argIndex: number) {
    return this.supportsParamCoercion && (argIndex <= this.minArity || argIndex <= this.maxArity);
  }
  ...
  private checkInvocationCore(
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (typeChecks && coercionType != null) {
        CollectionUtils.AddDictionary(nodeToCoercedTypeMap, args[i], coercionType);
      }
  ...
  /// </summary>
  public postVisitValidation(binding: TexlBinding, callNode: CallNode): boolean {
    return false;
  }
  ...
  public requiresPagedDataForParam(callNode: CallNode, paramIndex: number, binding: TexlBinding) {
    ...
    return this.requiresPagedDataForParamCore(callNode.args.children, paramIndex, binding);
  }
  ...
  public tryGetParamDescription(paramName: string): [boolean, string] {
    ...
    return StringResources.TryGet('About' + this.localeInvariantName + '_' + paramName);
  }
  ...
  public isServerDelegatable(callNode: CallNode, binding: TexlBinding): boolean {
    // Contracts.AssertValue(callNode);
    // Contracts.AssertValue(binding);
  ...
  public supportsPaging(callNode: CallNode, binding: TexlBinding): boolean {
    ...
    return binding.isDelegatable(callNode) || this.isServerDelegatable(callNode, binding);
  }
  ...
  public tryGetDataSource(
    ...
    var arg0 = args[0];
    return ArgValidators.DelegatableDataSourceInfoValidator.tryGetValidValue(arg0, binding);
  }
  ...
  public tryGetDataSourceNodes(
    ...
    const args = callNode.args.children;
    const arg0 = args[0];
    return ArgValidators.DataSourceArgNodeValidator.tryGetValidValue(arg0, binding);
  }

  // Returns a entityInfo for a function if function operates on entity.
  public tryGetEntityInfo(callNode: CallNode, binding: TexlBinding): [boolean, IExpandInfo] {
    // Contracts.AssertValue(callNode);
    // Contracts.AssertValue(binding);
  ...
  public tryGetFlowInfo(callNode: CallNode, binding: TexlBinding): [boolean, IFlowInfo] {
    // Contracts.AssertValue(callNode);
    // Contracts.AssertValue(binding);
  ...
  public getIdentifierOfModifiedValue(args: TexlNode[]): [Array<Identifier>, TexlNode] {
  ...
  protected static Arg0RequiresAsync(callNode: CallNode, binding: TexlBinding): boolean {
    // Contracts.AssertValue(callNode);
    // Contracts.AssertValue(binding);
  ...
  private setErrorForMismatchedColumnsCore(
    ...
    for (const expectedColumn of expectedType.getAllNames(DPath.Root)) {
      ...
      if (result[0]) {
        ...
        let errName: string;
        const rst = DType.TryGetDisplayNameForColumn(expectedType, expectedColumn.name.toString());
        errName = rst[1];
        if (!rst[0]) {
  ...
  protected checkTypeWithCoerce(
    ...
    const targetDifference = coercionType == null ? schemaDifference : coercionDifference;
    ...
    if ((targetType.isTable && nodeType.isTable) || (targetType.isRecord && nodeType.isRecord)) {
      if (this.setErrorForMismatchedColumns(expectedType, targetType, node, errors)) {
  ...
  protected getGenericSignatures(arity: number, ...args: StringGetter[]): Array<StringGetter[]> {
  ...
  protected suggestDelegationHint(node: TexlNode, binding: TexlBinding, telemetryMessage?: string) {
  ...
  protected checkParamIsTypeOrSingleColumnTable(
    ...
    errors: IErrorContainer,
    nodeToCoercedTypeMap: Dictionary<TexlNode, DType>
  ): [boolean, { isTable: boolean; nodeToCoercedTypeMap: Dictionary<TexlNode, DType> }] {
            ...
            this.name +
              (signature == null
                ...
                : '(' +
                  signature
                    .map((getter) => getter())
                    .join(TexlLexer.LocalizedInstance.localizedPunctuatorListSeparator + ' ') +
                  ')'),
