...
export class FormulaValueStatic {
  ...
  public static New(value?: any): any {
    ...
    if (typeof value === 'number') {
      return new NumberValue(IRContext.NotInSource(FormulaType.Number), value);
    }
  ...
  public static FromJsonString(jsonString: string) {
    try {
      ...
      return FormulaValueStatic.FromJson(json);
    } catch (e) {
      throw e;
    }
  }
    ...
    switch (valueKine) {
      ...
      case 'object':
        ...
        if (Array.isArray(element)) {
          //数组
          return FormulaValueStatic.TableFromJsonArray(element);
        }
        return FormulaValueStatic.RecordFromJsonObject(element);
      default:
  ...
  public static TableFromRecords(values: RecordValue[], type?: TableType): TableValue {
    if (type == null) {
      ...
      return FormulaValueStatic.TableFromRecords(values, recordType.toTable());
    }
  ...
  public static GuaranteeRecord(rawVal: FormulaValue): RecordValue {
    ...
    return FormulaValueStatic.RecordFromFields([defaultField]);
  }
  ...
  public static TableFromJsonArray(array: any): TableValue {
    ...
    for (let i = 0; i < Array.from(array).length; ++i) {
      ...
      const val = FormulaValueStatic.GuaranteeRecord(FormulaValueStatic.FromJson(element));
      records.push(val);
    }
    ...
    if (records.length == 0) {
      ...
      type = TableType.FromRecord(
        <RecordType>FormulaValueStatic.GuaranteeRecord(records[0]).irContext.resultType
      );
    }
    return new InMemoryTableValue(
      IRContext.NotInSource(type),
