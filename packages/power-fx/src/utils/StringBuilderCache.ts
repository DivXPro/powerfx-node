...
/// </example>
export class StringBuilderCache<T> {
  // The value 360 was chosen in discussion with performance experts as a compromise between using
  // as litle memory (per thread) as possible and still covering a large part of short-lived
  // StringBuilder creations on the startup path of VS designers.
  private static MaxBuilderSize = 360

  private static CachedInstance: StringBuilder = new StringBuilder()
  ...
  /// </remarks>
  public static SetMaxBuilderSize(maxSize: number) {
    // Contracts.CheckParam(maxSize > 0, nameof(maxSize));
    StringBuilderCache.MaxBuilderSize = maxSize
  }

  public static Acquire(capacity: number): StringBuilder {
    if (capacity <= StringBuilderCache.MaxBuilderSize) {
      const sb = StringBuilderCache.CachedInstance
  ...
  public static Release(sb: StringBuilder) {
    if (sb.capacity <= StringBuilderCache.MaxBuilderSize) {
      StringBuilderCache.CachedInstance = sb
      sb.clear()
    }
  }

  public static GetStringAndRelease(sb: StringBuilder) {
