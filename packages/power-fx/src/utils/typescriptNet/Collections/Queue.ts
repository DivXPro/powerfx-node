...
export class Queue<T> extends CollectionBase<T> {
  ...
  constructor(source?: FiniteEnumerableOrArrayLike<T> | number, equalityComparer: EqualityComparison<T> = areEqual) {
    ...
    if (!source) this._array = emptyArray
    else {
  ...
  protected _clearInternal(): number {
    const _ = this
    const array = _._array,
      head = _._head,
      tail = _._tail,
      size = _._size
    if (head < tail) AU.clear(array, head, tail)
    else {
      AU.clear(array, head)
  ...
  tryDequeue(out: Action<T>): boolean {
    ...
    if (this._tryDequeueInternal(out)) {
      ...
      if (_._size < _._capacity / 2) _.trimExcess(SHRINK_THRESHOLD)
  ...
  trimExcess(threshold?: number): void {
    ...
    const size = _._size
    if (
      size < Math.floor(_._capacity * 0.9) &&
      ((!threshold && threshold !== 0) || isNaN(threshold) || threshold < size)
    )
      _.setCapacity(size)
  }
