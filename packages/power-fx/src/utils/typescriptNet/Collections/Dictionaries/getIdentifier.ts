...
export function getIdentifier(
  obj: any,
  unknownHandler: Selector<any, string | number | symbol>,
): string | number | symbol
export function getIdentifier(
  obj: any,
  throwIfUnknown: boolean | Selector<any, string | number | symbol> = false,
): string | number | symbol | never {
  ...
  if (throwIfUnknown) {
    if (Type.isFunction(throwIfUnknown)) return throwIfUnknown(obj)
    else throw 'Cannot create known identity.'
  }

  return typeof obj.toString == Type.FUNCTION ? obj.toString() : Object.prototype.toString.call(obj)
}
