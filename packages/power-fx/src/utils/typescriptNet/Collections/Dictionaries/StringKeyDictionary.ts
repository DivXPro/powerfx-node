...
export class StringKeyDictionary<TValue>
  extends DictionaryBase<string, TValue>
  implements IStringKeyDictionary<TValue>
{
  ...
  containsKey(key: string): boolean {
    return key != null && this._count != 0 && this._map[key] !== VOID0
  }
  ...
  containsValue(value: TValue): boolean {
    ...
    const map = this._map
    for (let key in map) {
      if (map.hasOwnProperty(key) && areEqual(map[key], value)) return true
    }
    return false
  }

  getValue(key: string): TValue | undefined {
    return key == null || !this._count ? VOID0 : this._map[key]
  }
    ...
    if (_._count)
      for (let key in _._map) {
        if (_._map.hasOwnProperty(key)) {
          // This simply satisfies inspection.
