...
export class OrderedStringKeyDictionary<TValue>
  ...
  implements IOrderedDictionary<string, TValue>
{
  ...
  getValueByIndex(index: number): TValue {
    ...
    const o = this._order
    if (index < o.length) return this.getAssuredValue(o[index])
  ...
  setValue(
    ...
    if (!exists && (value !== VOID0 || keepIndex)) _._order.push(key)
    else if (exists && value === VOID0 && !keepIndex) remove(_._order, key)
