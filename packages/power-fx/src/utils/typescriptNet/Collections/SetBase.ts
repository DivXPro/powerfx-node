...
export abstract class SetBase<T> extends CollectionBase<T> implements ISet<T>, IDisposable {
  ...
  intersectWith(other: FiniteEnumerableOrArrayLike<T>): void {
    ...
    if (other instanceof SetBase) {
      ...
      if (s)
        s.forEach((n) => {
          if (!other.contains(n.value) && _._removeInternal(<any>n.value)) _._incrementModified()
        }, true)
  ...
  setEquals(other: FiniteEnumerableOrArrayLike<T>): boolean {
    ...
    return (
      this.getCount() ==
        (other instanceof SetBase ? other.getCount() : using(this.newUsing(other), (o) => o.getCount())) &&
  ...
  getEnumerator(): FiniteIEnumerator<T> {
    ...
    _.throwIfDisposed()
    const s = _._set
    return s && _.getCount() ? LinkedNodeList.valueEnumeratorFrom<T>(s) : EmptyEnumerator
  }
  ...
  protected _removeNode(node: ILinkedNodeWithValue<T> | null | undefined): boolean {
    return !!node && this.remove(<any>node.value) != 0
  }
