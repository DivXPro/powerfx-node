...
function yielder(recycle?: Yielder<any>): Yielder<any> | void {
  if (!yielderPool) yielderPool = ObjectPool.createAutoRecycled(() => new Yielder<any>(), 40)
...
class Yielder<T> implements IYield<T>, IRecyclable {
  ...
  } // this class is not entirely local/private.  Still needs protection.
...
export class EnumeratorBase<T> extends DisposableBase implements IEnumerator<T> {
  ...
  constructor(
    ...
    super(NAME)
    this.reset()
    if (Type.isBoolean(isEndless)) this._isEndless = isEndless
    else if (Type.isBoolean(disposer)) this._isEndless = disposer
    else this._isEndless = false

    if (Type.isFunction(disposer)) this._disposer = disposer
  }
  ...
  next(): IIteratorResult<T> {
    return this.moveNext() ? new IteratorResult(this.current, this.index) : IteratorResult.Done
  }
