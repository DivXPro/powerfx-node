...
function initArrayFrom(source: FiniteEnumerableOrEnumerator<any>, max: number = Infinity): any[] {
...
export function from<T>(
  source:
    | FiniteEnumerableOrArrayLike<T>
    | EnumerableOrArrayLike<T>
    | IEnumerator<T>
    | InfiniteValueFactory<T>
    | IIterator<T>
    | null
    | undefined,
): IEnumerator<T> {
  ...
  if (source instanceof Array) return new ArrayEnumerator<T>(<T[]>source)
  ...
  if (!Type.isPrimitive(source)) {
    if (isEnumerable<T>(source)) return source.getEnumerator()

    if (Type.isFunction(source)) return new EndlessEnumerator(source)

    if (isEnumerator<T>(source)) return source

    if (isIterator<T>(source)) return new IteratorEnumerator<T>(source)
  }
...
export function forEach<T>(e: FiniteEnumerableOrEnumerator<T>, action: ActionWithIndex<T>, max?: number): number

export function forEach<T>(e: FiniteEnumerableOrEnumerator<T>, action: PredicateWithIndex<T>, max?: number): number
  ...
  if (e && max > 0) {
    ...
    if (isEnumerator<T>(e)) {
      ...
      while (max > i && e.moveNext()) {
        if (action(<any>e.current, i++) === false) break
      }
      return i
    }
    ...
    if (isEnumerable<T>(e)) {
      ...
      return using((<FiniteIEnumerable<T>>e).getEnumerator(), (f) => forEach(f, action, max))
    }
  }
 ...
 * @returns {any}
 */
export function toArray<T>(source: FiniteEnumerableOrEnumerator<T>, max: number = Infinity): T[] {
  if (<any>source === STRING_EMPTY) return []

  if (!isFinite(max) && source instanceof Array) return source.slice()
...
export function map<T, TResult>(
  ...
  if (!isFinite(max) && source instanceof Array) return source.map(selector)
