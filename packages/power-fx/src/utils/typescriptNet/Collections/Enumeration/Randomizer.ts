...
export class Randomizer<T> extends SimpleEnumeratorBase<T> {
  ...
  constructor(source: FiniteIEnumerable<T>, private readonly _allowReset: boolean = false) {
    super()
  ...
  reset(): void {
    if (this._allowReset) {
      ...
      this._pointer = this._buffer.length
      super.reset()
    } else throw 'Reset not allowed.  To allow for reset, specify so when constructing.'
  }
