  ...
  implements ILinkedNodeList<TNode>, IEnumerateEach<TNode>, IDisposable, IRecyclable
{
  ...
  forEach(action: ActionWithIndex<TNode>, ignoreVersion?: boolean): number
  forEach(action: PredicateWithIndex<TNode>, ignoreVersion?: boolean): number
  forEach(action: ActionWithIndex<TNode> | PredicateWithIndex<TNode>, ignoreVersion?: boolean): number {
    const _ = this
    ...
    } while (current && <any>action(current, index++) !== false)
  ...
  clear(): number {
    ...
    if (cF !== cL)
      console.warn(
        'LinkedNodeList: Forward versus reverse count does not match when clearing. Forward: ' +
          cF +
          ', Reverse: ' +
          cL,
      )
  ...
  takeFirst(): TNode | undefined {
    ...
    if (node.previous) throw new Exception('Collection is corrupted: first node has previous node.')
    if (!this.removeNode(node)) throw new Exception('Collection is corrupted: unable to remove first node.')
    return node
  }
  ...
  takeLast(): TNode | undefined {
    ...
    if (!node) return VOID0
    if (node.next) throw new Exception('Collection is corrupted: last node has next node.')
    if (!this.removeNode(node)) throw new Exception('Collection is corrupted: unable to remove last node.')
    return node
  }
  ...
  static valueEnumeratorFrom<T>(list: LinkedNodeList<ILinkedNodeWithValue<T>>): FiniteIEnumerator<T> {
    ...
    let current: ILinkedNodeWithValue<T> | undefined, next: ILinkedNodeWithValue<T> | undefined, version: number
...
function assertValidDetached<TNode extends ILinkedNode<TNode>>(node: TNode, propName: string = 'node') {
  if (!node) throw new ArgumentNullException(propName)
