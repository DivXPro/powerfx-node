...
const bucketPool = ObjectPool.createAutoRecycled(() => new LinkedNodeList<any>(), 100)
...
export class HashSet<T> extends SetBase<T> {
  ...
  constructor(source: any, keyGenerator: any = selfHash, comparer: EqualityComparison<T> = areEqual) {
    ...
    if (Type.isFunction(source)) {
      ...
      if (!keyGenerator) throw new ArgumentNullException('keyGenerator')
      this._hashGenerator = keyGenerator
      this._comparer = comparer
      this._importEntries(source)
    }
    if (!this._comparer) throw new ArgumentNullException('comparer')
  }
  ...
  protected _removeInternal(item: T, max: number = Infinity): number {
    ...
    if (node) {
      ...
      const s = this._set
      if (s && s.removeNode(listNode)) return 1
    }
...
function wipe(map: IMap<any> | undefined, depth: number = 1): void {
  if (map && depth) {
    for (let key of Object.keys(map)) {
      let v = map[key]
      delete map[key]
      if (v instanceof LinkedNodeList) bucketPool.give(v)
      else wipe(v, depth - 1)
    }
  }
}
