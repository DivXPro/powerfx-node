...
export abstract class CollectionBase<T> extends DisposableBase implements ICollection<T>, IEnumerateEach<T> {
  ...
  protected assertModifiable(): true | never {
    ...
    if (this.getIsReadOnly()) throw new InvalidOperationException(CMRO)
    return true
  }
  ...
  toArray(): T[] {
    const count = this.getCount()
    return count ? this.copyTo(count > 65536 ? new Array<T>(count) : []) : []
  }
}
