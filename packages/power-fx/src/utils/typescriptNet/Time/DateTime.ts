...
export class DateTime implements ICalendarDate, IDateTime, IEquatable<IDateTime>, IComparable<IDateTime> {
  ...
  constructor(value: any = new Date(), kind: DateTime.Kind = DateTime.Kind.Local) {
    ...
    if (value instanceof DateTime) {
      ...
      if (value instanceof Date) {
        ...
        this._value = value === VOID0 ? new Date() : new Date(value)
      }
    }
  }
   ...
   * @returns {DateTime}
   */
  // subtract(time: DateTime): DateTime
  subtract(time: TimeSpan): DateTime
  subtract(time: DateTime): TimeSpan
  subtract(time: DateTime | TimeSpan): DateTime | TimeSpan {
    if (time instanceof TimeSpan) {
      return this.addMilliseconds(-time.getTotalMilliseconds())
    }
    return DateTime.between(time, this)
  }
  ...
  static TryParse(str: string): [boolean, DateTime] {
    const date = Date.parse(str)
    return [!isNaN(date), isNaN(date) ? undefined : new DateTime(date)]
  }
  ...
  toString(formatter?: string) {
    return dayjs(this.value).format(formatter)
  }
}
